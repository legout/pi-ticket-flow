...
# Ralph-Wiggum One-Ticket Orchestrator
...
- `rw/current.md` — orchestrator state for the current ticket
- `rw/<ticket-id>/implementation.md` — worker output
- `rw/<ticket-id>/review.md` — reviewer output

### `rw/current.md` format
...
3. If it exists and `stage: waiting-worker`:
   ...
   - if the implementation artifact contains `status: blocked`, escalate immediately, mark `rw/current.md` as `done`, and stop
...
7. If the chosen ticket is not already `in_progress`, run:
   - `tk start <ticket-id>`
8. Write `rw/current.md` with `stage: waiting-worker`
...
- write `rw/<ticket-id>/implementation.md`
...
10. Update `rw/current.md` to `stage: done`
11. Stop
