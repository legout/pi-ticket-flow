---
description: Direct fallback Ralph-Wiggum implementation — process exactly one tk ticket end-to-end
...
First try to read `rw/current.md` using `read_artifact`.

When parsing `rw/current.md`, require exactly one line each with these exact prefixes:
- `ticket:`
- `ticket_path:`
- `stage:`
...
- if the implementation artifact exists and contains `status: blocked`:
  ...
  - update `rw/current.md` so `stage: done`
  - stop immediately
...
If `rw/current.md` exists and contains `stage: done`, ignore it and continue to ticket selection.

If `rw/current.md` does not exist, continue to ticket selection.
...
Write `rw/current.md` with this exact shape:
