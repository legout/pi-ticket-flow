...
10. Otherwise parse `gate: PASS` or `gate: REVISE` from the review artifact.
11. Run `tk notes <ticket>` and count prior notes containing `Gate: REVISE`.
12. If gate is PASS:
...
13. If gate is REVISE and this is failure 1 or 2:
...
14. If gate is REVISE and this is failure 3:
   - add a concise structured ESCALATE note via `tk add-note <ticket> ...`
   - include `Gate: ESCALATE`
   - leave the ticket `in_progress`
   - treat the queue outcome as `ESCALATE`
15. Overwrite `ticket-flow/current.md` with:
...
16. Update `ticket-flow/progress.md` by preserving `started_at` and existing history, then setting:
   - `status: waiting-next`
   - `last_updated: <now>`
   - `current_ticket: none`
...
17. Update `ticket-flow/lessons-learned.md` only if there is a durable, reusable, non-duplicate lesson from this ticket.
...
18. End with a short summary including the ticket id and final queue outcome.
